...
namespace Bonsai.Validators
{
    public static class TextValidator
    {
        ...
        private static readonly string SYMBOL_PATTERN = @".*[!\(\)\-\?.,\[\]_'""`~#$%^&*+=]+.*";
        ...
        public static bool ContainsSymbols(string word)
        {
            ...
            return Regex.IsMatch(word, SYMBOL_PATTERN);
        }

        public static bool FirstCharacterIsSymbol(string word)
        {
            //return word.Any(c => char.IsSymbol(c));
            return Regex.IsMatch(word.Substring(0, 1), SYMBOL_PATTERN);
        }
